...
export default class PlayerSwordRunningState extends State {
	/**
	...
	enter() {
		...
		this.player.sprites = Player.generateSprites(ImageName.SwordRunning,Player.SWORD_RUNNING)
	}
	...
	update(dt) {
        ...
        if (this.player.currentAnimation.getCurrentFrame() == 7 && this.shooting) {
			...
			this.player.sprites = Player.generateSpritesGunRunningAim()
            this.shooting =false
		}
