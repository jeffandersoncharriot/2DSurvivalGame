...
export default class PlayerRunningGunState extends State {
	/**
	...
	enter() {
		...
		this.player.sprites = Player.generateSprites(ImageName.GunRunningAim,Player.GUN_SPRITES_RUN)
	}
	...
	update(dt) {
        ...
        if((keys.e && keys.a) || (keys.e && keys.d))
        {

			this.player.shoot()
            keys.e= false
            this.player.sprites = Player.generateSprites(ImageName.GunRunningShoot,Player.GUN_SPRITES_RUN,Player.EXTRA_WIDTH)
            this.shooting=true
			
        }

        if (this.player.currentAnimation.getCurrentFrame() == 7 && this.shooting) {
